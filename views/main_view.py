...
from controllers.main_api_controller import MainViewApiController
...
class MainView(Screen):
    ...
    def show_create_invoice(self):
        ...
        self.api_controller.get_next_invoice_number(
            ...
            force_refresh=False  # Only set to True if you need fresh data from server
        )

    def refresh_invoice_numbers(self):
        """Force refresh the invoice numbers from server"""
        if self.api_controller:
            self.api_controller.invalidate_cache()
